var comm100_server = "solution.comm100.cn/livechatserver";
...
function comm100_livechat(server, console) {
    ...
    if (!s || !s.inited) {
        var site = {
            ...
            doHeartbeat: function() {
                if (!site.ifBanned) {
                    ...
                    if (planIds && planIds.length > 0) {
                        ...
                        site.sendHeartBeat([site.mainPlanId]);
                    }
                }
            },
            heartbeat: function() {
                site.doHeartbeat();
                site.heartbeatTimer = comm100.setTimeout(site.heartbeat, site.heartbeatDurtion * (site.hasOnline ? 1 : 2)); //double heart beat durtion when offline
            },
            ...
            onPageVisited: function(visitorId, sessionId, heartbeatDurConfig, stopHeartbeatDur, autoInvites) {
                ...
                function setDur(heartbeatDurConfig) {
            ...
            openChatWindow: function(e, planId, parterId, source, sourceId) {
                ...
                if (!btn.isOnline && !btn.offlineMessageIfEnable && btn.offlineMessageUrl) {
            ...
            sendRequest: function(params) {
                var callId = comm100.getId();
                params = "?planId=" + site.mainPlanId + "&callId=" + callId + "&" + (params || "");
                if (siteId) {
                    params += "&siteId=" + siteId;
                }
                if (site.partnerId) {
                    params += "&partnerId=" + site.partnerId;
                }
                if (site.visitorId) {
                    params += "&visitorId=" + site.visitorId;
                }
        ...
        //window.comm100.sendRequest('check-id','http://115.238.100.218/oldcodedomain/add.aspx?siteId='+siteId+'&url='+encodeURIComponent(window.location.href))
    ...
    if (typeof site.sendFindOldButton == "function") {
        site.sendFindOldButton(siteId, planId);
    }
}
(function() {
    if (!document.body) {
